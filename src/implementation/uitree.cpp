...
namespace sfe{
    ...
    static void draw_operand(const VisualASTNode& operand,const GraphicsContext& gc,const bbe::ErrorDatabase& errors,const sfe::NameDatabase& names,const UICursor& cursor,cppp::fvec2& pos,std::uint32_t my_priority){
    ...
    static void draw_binop(std::u8string_view op,const std::vector<VisualASTNode>& children,const GraphicsContext& gc,const bbe::ErrorDatabase& errors,const sfe::NameDatabase& names,const UICursor& cursor,cppp::fvec2& pos,std::uint32_t my_priority){
    ...
    void VisualASTNode::draw(const GraphicsContext& gc,const bbe::ErrorDatabase& errors,const sfe::NameDatabase& names,const UICursor& cursor,cppp::fvec2& pos) const{
        ...
        switch(type()){
            ...
            case ARG:
                ...
                break;
            case FNSYM:
                gc.draw_text_at_cursor(names.display_function_name(p32()),pos,1.0f,WHITE);
                cursor_pos = pos;
                break;
            case BOOL:
            ...
            case CALL_BUILTIN:
                switch(p32()){
                    ...
                    case 60:
                        ...
                        draw_operand(_children.front(),gc,errors,names,cursor,pos,priority());
                        break;
                    default:
    ...
    void VisualFunctionNode::draw(const GraphicsContext& gc,const bbe::ErrorDatabase& errors,const sfe::NameDatabase& names,const UICursor& cursor,cppp::fvec2& pos) const{
        float top_y = pos.y()-gc.ascender();
        float left_x = pos.x();
        float right_x;
        {
            cppp::fvec2 line_1{pos};
            gc.draw_text_at_cursor(cppp::format<u8"{}:"_ts>(names.display_function_name(id)),line_1,1.0f,WHITE);
            right_x = line_1.x();
        }
        pos += cppp::fvec2(gc.indentation()*4.0f,gc.line_height());
        _child.draw(gc,errors,names,cursor,pos);
